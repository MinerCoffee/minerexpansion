...
public class CommandSupplyDrop implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        if (sender instanceof Player) {
                ...
                if (p.hasPermission("supplydrop.use")) {
                    ...
                    int locX = (int) ((double) Integer.signum((int) ploc.getX()) == -1.0D ? ploc.getX() - 1.0D : ploc.getX());
                    int locY = (int) ploc.getY();
                    int locZ = (int) ((double) Integer.signum((int) ploc.getZ()) == -1.0D ? ploc.getZ() - 1.0D : ploc.getZ());

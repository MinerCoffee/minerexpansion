    ...
    public void perform(CommandSender sender, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length > 1 && p.hasPermission("miner.staff")) {
                ...
                if (amount <= 0) {
                    p.sendMessage(ChatUtils.colour("&4You must deposit a value greater than zero."));
                } else {
                    EconomyResponse response = MinerExpansion.getEconomy().depositPlayer(p, amount);
                    if (response.type == EconomyResponse.ResponseType.SUCCESS) {
                        p.sendMessage(ChatUtils.colour("&b" + amount + " has been deposited into your account. The new total is " + response.balance));
                    } else {
                        p.sendMessage(ChatUtils.colour("&4Your deposit could not be processed. Try again later."));
                    }
                }
            }
            } else {
                p.sendMessage(ChatUtils.colour("&4You must deposit a value greater than zero."));
                p.sendMessage(ChatUtils.colour("&aExample: &e/admin deposit 123.4"));
            }
